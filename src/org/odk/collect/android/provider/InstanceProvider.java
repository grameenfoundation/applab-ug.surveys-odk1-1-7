...
public class InstanceProvider extends ContentProvider {
    ...
    private static final String ALTER_INSTANCES_ADD_INTERVIEWEE = 
        "ALTER TABLE instances ADD  interviewee";
    private static final String ALTER_INSTANCES_ADD_LOCATION = 
        "ALTER TABLE instances ADD location TEXT";
    ...
    private static class DatabaseHelper extends ODKSQLiteOpenHelper {
        ...
        public void onCreate(SQLiteDatabase db) { 
           ...
           db.execSQL("CREATE TABLE " + INSTANCES_TABLE_NAME + " (" 
               ...
               + InstanceColumns.INTERVIEWEE + " text not null, "
               + InstanceColumns.LOCATION  + " text not null );");  
           
           Log.d("DB CREATED", "Table Instances created");
           Cursor c = db.query(INSTANCES_TABLE_NAME, new String[]{"interviewee"}, null, null, null, null, null);
           Log.d("Cursor Size", "After DB creation is "+String.valueOf(c.getCount()));
           c.close();
        }
    ...
    public Cursor query(Uri uri, String[] projection, String selection, String[] selectionArgs,
        ...
        Log.d("DB VERSION", String.valueOf(db.getVersion()));
    ...
    public Uri insert(Uri uri, ContentValues initialValues) {
        ...
        if (values.containsKey(InstanceColumns.LOCATION) == false) {
            values.put(InstanceColumns.LOCATION, GpsManager.getInstance().getLocationAsString());
            Log.d("LOCATION", GpsManager.getInstance().getLocationAsString());
        }
        
        if (values.containsKey(InstanceColumns.INTERVIEWEE) == false) {
            values.put(InstanceColumns.INTERVIEWEE, GlobalConstants.intervieweeName);
            Log.d("INTERVIEWEE", GlobalConstants.intervieweeName);
        }
        
        if (values.containsKey(InstanceColumns.SUBMISSION_URI) == false) {
            values.put(InstanceColumns.SUBMISSION_URI, GpsManager.getInstance().getLocationAsString() + "," +GlobalConstants.intervieweeName);
            Log.d("INTERVIEWEE", GlobalConstants.intervieweeName);
        }

        SQLiteDatabase db = mDbHelper.getWritableDatabase();
        Cursor d = db.query(INSTANCES_TABLE_NAME, new String[]{"interviewee"}, null, null, null, null, null);
        Log.d("CURSOR SIZE", String.valueOf(d.getCount()));
       /* for (int x = 0; x < d.getColumnCount(); x++) {
        	Log.d("COLUMN"+x, d.getString(x));
        }*/
        d.close();
        long rowId = db.insert(INSTANCES_TABLE_NAME, null, values);
        Cursor d2 = db.query(INSTANCES_TABLE_NAME, new String[]{"interviewee"}, null, null, null, null, null);
        Log.d("CURSOR SIZE", "AFTER INSERT "+String.valueOf(d.getCount()));
        if (rowId > 0) {
    ...
    static {
        ...
        sInstancesProjectionMap.put(InstanceColumns.LOCATION, InstanceColumns.LOCATION);
        sInstancesProjectionMap.put(InstanceColumns.INTERVIEWEE, InstanceColumns.INTERVIEWEE);
    }
